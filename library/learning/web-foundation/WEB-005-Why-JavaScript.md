...
JavaScript는 웹페이지에 동작과 상호작용을 더하는 프로그래밍 언어입니다. HTML이 구조를 만들고 CSS가 표현과 배치를 입힌다면, JavaScript는 사용자의 입력에 반응하고 화면 상태를 바꾸며 필요할 때 서버/API와 데이터를 주고받습니다.

핵심 질문은 이것입니다.

> 왜 HTML과 CSS만으로 충분하지 않고 JavaScript가 필요해졌는가?
...
HTML과 CSS는 기본적으로 “무엇을 보여줄지”와 “어떻게 보여줄지”에 강합니다. 반면 “사용자가 무엇을 했을 때 어떤 일이 일어나야 하는지”를 다루려면 프로그램이 필요합니다. JavaScript는 이 지점에서 웹을 정적인 문서에서 상호작용 가능한 웹앱으로 확장합니다.

## 3. 핵심 개념

### JavaScript

JavaScript는 브라우저 안에서 실행될 수 있는 프로그래밍 언어입니다. 웹페이지의 상태를 바꾸고, 사용자 입력에 반응하고, HTML 요소를 찾거나 수정하고, 서버/API와 데이터를 주고받을 수 있습니다.
...
DOM은 브라우저가 HTML 문서를 읽어 만든 문서 구조입니다. JavaScript는 DOM에서 요소를 찾고, 텍스트를 바꾸고, 클래스를 추가하거나 제거하고, 새 요소를 만들 수 있습니다. 이처럼 JavaScript로 DOM을 수정하는 일을 DOM manipulation이라고 부릅니다.
...
이 코드는 문서 안의 `button` 요소를 찾고, 버튼 클릭 event가 발생하면 `body`에 `is-active` 클래스를 켜고 끕니다. JavaScript 문법 전체를 외우는 것보다 먼저 보아야 할 점은 “요소를 찾고, event를 듣고, 화면 상태를 바꾼다”는 흐름입니다.

### Browser Execution

브라우저는 HTML과 CSS만 읽는 도구가 아닙니다. JavaScript 파일도 내려받아 실행하는 Runtime입니다. 브라우저가 제공하는 DOM, event, fetch 같은 기능 덕분에 JavaScript는 웹페이지와 서버/API 사이를 연결할 수 있습니다.
...
2. CSS가 HTML 구조 위에 시각적 표현과 배치를 적용한다.
3. 브라우저가 JavaScript 파일을 읽고 실행한다.
4. JavaScript가 버튼 클릭, 키보드 입력, 마우스 이동 같은 사용자 이벤트를 감지한다.
5. 이벤트가 발생하면 JavaScript가 상태를 바꾸거나 DOM을 수정한다.
6. 필요한 경우 JavaScript가 서버/API에 HTTP 요청을 보낸다.
7. 응답 데이터를 받아 화면을 업데이트한다.
8. 최종적으로 정적인 문서가 상호작용 가능한 웹앱이 된다.
...
GitHub Pages 프로젝트에서도 이 흐름은 같습니다. `index.html`이 구조를 만들고, `style.css`가 화면을 정리하며, `app.js` 같은 JavaScript 파일이 문서 목록을 불러오고 클릭에 반응하고 화면 내용을 바꿉니다.
...
Potato's Day에서 JavaScript는 버튼 클릭을 감지하고, 감자의 상태를 바꾸고, 메시지를 출력하고, sprite animation을 전환하는 역할을 맡을 수 있습니다. CSS가 감자의 집과 버튼을 보기 좋게 배치한다면 JavaScript는 사용자의 행동에 따라 장면이 반응하도록 만듭니다.

### Living Aegis Origin

Living Aegis Origin에서 JavaScript는 Canvas를 그리고, 게임 루프를 돌리고, 입력을 처리하고, 충돌을 계산하고, HUD 상태를 갱신할 수 있습니다. 즉 게임이 “그림”으로 멈춰 있지 않고 계속 판단하고 반응하게 만드는 중심입니다.
...
2. HTML 문서에 버튼 하나가 있을 때 JavaScript가 그 버튼을 어떻게 찾고 event를 연결할지 흐름을 말로 적어봅니다.
3. “메뉴가 열려 있다/닫혀 있다”처럼 화면 상태로 볼 수 있는 값을 3개 적어봅니다.
4. 개발자 도구의 Elements 탭에서 버튼을 클릭했을 때 클래스나 텍스트가 바뀌는 웹페이지를 관찰합니다.
5. API에서 데이터를 받아 화면을 바꾸는 서비스를 하나 떠올리고, 어떤 데이터가 state가 될 수 있는지 적어봅니다.

## 8. 다음 문서와의 연결

- WEB-006에서는 HTML, CSS, JavaScript 파일과 API 데이터가 HTTP 요청과 응답으로 오가는 규칙을 더 자세히 학습합니다.
...
- JavaScript 문법, module, async/await, Canvas API는 이후 프로젝트 경험이 쌓일 때 별도 Guide나 Reference로 나눌 수 있습니다.
