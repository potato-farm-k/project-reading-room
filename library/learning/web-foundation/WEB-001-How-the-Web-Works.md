...
- 웹을 Request와 Response가 반복되는 시스템으로 이해합니다.
- Browser, URL, DNS와 HTTP가 한 번의 페이지 요청에서 어떻게 연결되는지 설명합니다.
- DNS와 DDNS가 각각 어떤 문제를 해결하는지 구분합니다.
...
웹을 이해하는 가장 중요한 문장 중 하나는 다음과 같습니다.

> 웹은 수많은 컴퓨터가 **Request와 Response**를 반복하며 동작하는 시스템입니다.

브라우저가 요청하고 서버가 응답합니다. 이 관점은 이후 HTTP, REST API, CORS, Serverless를 이해할 때도 계속 이어집니다.
...
인터넷 위에서 URL과 HTTP를 사용해 문서와 데이터를 연결하고 주고받는 시스템입니다. 인터넷은 네트워크 기반이고, 웹은 그 기반 위에서 동작하는 서비스 중 하나입니다. 웹의 기본 흐름은 Client가 Request를 보내고 Server가 Response를 돌려주는 것입니다.

### Client

자원을 요청하는 쪽입니다. 브라우저, 모바일 앱 또는 API를 호출하는 프로그램이 Client가 될 수 있습니다. Client는 “무엇을 원한다”는 요청을 시작합니다.

### Server

요청을 받고 알맞은 문서나 데이터를 응답하는 쪽입니다. 정적 파일을 그대로 전달할 수도 있고, 요청에 따라 결과를 계산할 수도 있습니다. Server는 요청을 해석하고 가능한 결과를 Response로 돌려줍니다.
...
브라우저가 어느 DNS 서버에 물어봐야 하는지는 보통 운영체제가 네트워크 연결 과정에서 받은 DNS 서버 주소를 통해 알게 됩니다. 이 내용은 이후 DNS 심화 학습에서 다시 다룹니다.

### DDNS

DDNS(Dynamic DNS)는 공인 IP가 바뀔 수 있는 환경에서 도메인이 최신 IP를 가리키도록 돕는 방식입니다. 집에서 운영하는 서버처럼 IP가 자주 바뀔 수 있는 경우에 사용합니다.

DDNS는 하나의 서버가 모든 일을 알아서 처리하는 구조가 아닙니다. 보통 다음 두 요소가 함께 동작합니다.

- **DDNS Provider:** DNS 레코드를 관리하는 서비스
- **DDNS Client:** IP 변경을 감지하거나 확인해 Provider에 업데이트를 요청하는 프로그램

중요한 점은 업데이트를 시작하는 주체가 DDNS Client라는 것입니다. DNS 서버는 스스로 집 서버의 IP 변화를 알아내지 않습니다.
...
DDNS를 사용하는 환경에서는 중간에 IP 갱신 과정이 추가될 수 있습니다.
...
- **DDNS가 내 IP를 스스로 알아내는가?** 보통은 DDNS Client가 현재 IP를 확인하고 Provider에 갱신을 요청합니다.
...
직접 운영하는 중간 서버가 있다면 도메인, DNS, DDNS와 서버 주소 관리가 문제 해결의 일부가 될 수 있습니다.
...
5. 공유기나 운영체제 네트워크 설정에서 DNS 서버 주소가 어디에 기록되는지 찾아봅니다.
...
| 2026-07-08 | v2 개정판 반영: 웹을 Request/Response 시스템으로 설명하고 DNS/DDNS 역할을 보강 | 브라우저는 운영체제가 제공한 DNS 서버 주소를 어떤 순서로 사용해 이름을 해석하는가? |
