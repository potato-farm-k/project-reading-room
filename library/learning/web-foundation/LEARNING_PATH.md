...
| WEB-007 | HTTP | Client와 Server는 어떤 규칙으로 요청과 응답을 주고받는가? | `planned` | 학습 시 생성 |
| WEB-008 | REST API | 웹 자원과 작업을 일관된 인터페이스로 어떻게 표현하는가? | `planned` | 학습 시 생성 |
...
- 현재 파일이 있는 Guide는 `WEB-001`, `WEB-002`, `WEB-003`, `WEB-004`, `WEB-005`, `WEB-006`입니다.
- `WEB-007` 이후 문서는 해당 주제를 실제로 학습할 때 만듭니다.
...
[WEB-006. How Browser Builds a Page](./WEB-006-How-Browser-Builds-a-Page.md)에서 브라우저가 DOM, CSSOM, Render Tree를 만들고 화면을 구성하는 흐름을 살펴봅니다.
